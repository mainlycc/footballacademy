...
const Badge3DInternal: React.FC<Badge3DProps> = ({ url, spinTrigger, zoomLevel = 0, isLit, hideShadows = false, onPointerDown, onLoadComplete, adminMode = false, onSceneReady }) => {
      ...
      clone.traverse((child) => {
        if (child instanceof THREE.Mesh && child.material) {
          if (Array.isArray(child.material)) {
            child.material = child.material.map((m) => (m && (m as any).clone ? (m as any).clone() : m));
          } else {
            const m: any = child.material;
            child.material = m && m.clone ? m.clone() : m;
          }
        }
      });
    } else {
...
const Badge3D: React.FC<Badge3DProps> = React.memo(({ url, spinTrigger, zoomLevel = 0, isLit, hideShadows = false, onPointerDown, onLoadComplete, adminMode = false, onSceneReady }) => {
